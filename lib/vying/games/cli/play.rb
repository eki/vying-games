...
module CLI
  ...
  def self.play
    ...
    opts.banner = 'Usage: vg play [options]'
    ...
    opts.on('-p', '--player PLAYER=BOT') do |s|
      if md = s.match(/(\w*)=([\w:]*)/)
        b = Vying::Games::Bot.find(md[2])
        p = md[1].downcase.intern
        p2b[p] = (b ? b.new : Vying::Games::Human.new(md[2]))
      end
    end
    ...
    number.times do |n|
      ...
      until g.final?
        if g[g.turn].user.class == Vying::Games::Human
