...
class TestGame < Minitest::Test
  ...
  def test_undo_by_request_three_players
    ...
    assert_equal(%w(undo_accepted_by_white
    ...
    assert_equal(%w(undo_accepted_by_blue
                   time_exceeded_by_white
                   time_exceeded_by_red
                   time_exceeded_by_blue
                   cancel_undo
  ...
  def test_history_last_move
    ...
    assert_equal([], g.history.last_turn.map(&:to_s))

    g << 'h10'

    assert_equal(['h10'], g.history.last_turn.map(&:to_s))

    g << 'i12'

    assert_equal(['i12'], g.history.last_turn.map(&:to_s))

    g << 'h8'

    assert_equal(['h8'], g.history.last_turn.map(&:to_s))

    g << 'k14'

    assert_equal(['k14'], g.history.last_turn.map(&:to_s))

    g << 'h11h9'

    assert_equal(['h11h9'], g.history.last_turn.map(&:to_s))

    g << 'h9h7'

    assert_equal(%w(h11h9 h9h7), g.history.last_turn.map(&:to_s))

    i = g.history.moves.length - 1

    assert_equal(%w(h11h9 h9h7),
      g.history.last_turn(i).map(&:to_s))
