...
class TestDodgem < Minitest::Test
  ...
  def test_initial_moves
    ...
    assert_equal(%w(a2b2 a3a4 a3b3), g.moves.sort)

    g << 'a2b2'

    assert_equal(%w(b4a4 b4b3 c4c3), g.moves.sort)

    g << 'b4a4'

    assert_equal(%w(a3a2 a3b3 b2b3 b2c2), g.moves.sort)

    g << 'b2c2'

    assert_equal(%w(a4b4 c4b4 c4c3), g.moves.sort)

    g << 'c4c3'

    assert_equal(%w(a3a2 a3b3 c2d2), g.moves.sort)

    g << 'c2d2'

    assert_equal(%w(a4b4 c3b3 c3c2), g.moves.sort)
