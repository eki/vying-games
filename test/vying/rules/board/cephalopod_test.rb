...
class TestCephalopod < Minitest::Test
  ...
  def test_capture
    ...
    assert_equal(%w(b1 d1).sort, g.moves.sort)
    ...
    assert_equal(%w(c3 b2 d2 c1).sort, g.moves.sort)

    g << 'c3'

    assert_equal(%w(b2 d2 c1).sort, g.moves.sort)
  ...
  def test_illegal_captures_01
    ...
    g << %w(b1 d1 c2 c1 b1 c2 c1 c3 d2 b2 c2
  ...
  def test_illegal_captures_02
    g = Game.new(rules)
    g << %w(a1 a2 e2 b3 d2 b2 b3 a2 b2 b1 a1
