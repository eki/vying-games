...
class TestLinesOfAction < Test::Unit::TestCase
  ...
  def test_info
    assert_equal( "Lines of Action", LinesOfAction.info[:name] )
  end

  def test_players
    assert_equal( [:black,:white], LinesOfAction.players )
    assert_equal( [:black,:white], LinesOfAction.new.players )
  end

  def test_initialize       # Need to be more thorough here
    g = Game.new( LinesOfAction )
